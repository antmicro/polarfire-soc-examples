...
typedef enum
{
	...
	G5C_MESSAGE_PLIC		=82 + OFFSET_TO_MSS_GLOBAL_INTS,
	USOC_vc_interrupt_PLIC	=83 + OFFSET_TO_MSS_GLOBAL_INTS,
	USOC_smb_interrupt_PLIC	=84 + OFFSET_TO_MSS_GLOBAL_INTS,
	PLL_EVENT_PLIC			=85 + OFFSET_TO_MSS_GLOBAL_INTS,
...
extern void pse_printf(const char * s, ...);   //fixme- for debug- remove when finished

/*-----------------Hart1 Mode Enables--------------------*/

static inline void PLIC_init(void)
{
//fixme: add in support for all harts
    uint32_t inc;
    ...
    {
    	PLIC->HART0_MMODE_ENA[inc]= 0;
    }
#else
    PLIC->HART0_MMODE_ENA[0]= 0x01 << USART0_PLIC_4;
    PLIC->HART0_MMODE_ENA[1]= 0x01 << (ethernet_PLIC_53 % 32);
#endif

    for(inc = 0; inc < PLIC_SET_UP_REGISTERS; ++inc)
	{
		PLIC->HART1_MMODE_ENA[inc]=0;
		PLIC->HART1_SMODE_ENA[inc]=0;

		PLIC->HART2_MMODE_ENA[inc]=0;
		PLIC->HART2_SMODE_ENA[inc]=0;

		PLIC->HART3_MMODE_ENA[inc]=0;
		PLIC->HART3_SMODE_ENA[inc]=0;

		PLIC->HART4_MMODE_ENA[inc]=0;
		PLIC->HART4_SMODE_ENA[inc]=0;
	}
    ...
    * the priorities previously set by other harts to be messed up. */
    ...
    {
        PLIC->SOURCE_PRIORITY[inc] = 0; /* priority must be greater than threshold to be enabled, enable while testing */
    }
...
{
//fixme: add in support for all harts
    uint32_t current = PLIC->HART0_MMODE_ENA[IRQn / 32];
 ...
 *
 */
//PLIC_ClearPendingIRQ() TODO: Place holder, will be added on next release
